...
def EV_G_ITER(model, scaled_state, params, control):
    ...
    G_ = torch.empty(M)
    autarky_state = model.cfg["model"]["params"]["autarky_state"]
    ...
    G_[counter : counter + n_types] = state[S[f"w_{1}"] : S[f"w_{1}"] + n_types] + control[P[f"pen_{1}"] : P[f"pen_{1}"] + n_types] - control[P[f"pen_u_{1}"] : P[f"pen_u_{1}"] + n_types] - \
        torch.sum(torch.unsqueeze(control[P[f"u_{1}"]:P[f"u_{1}"]+n_types] + beta*(autarky_state[0, :] + control[future_util_diag_mask]),0)*trans_mat[:,:],-1)
    ...
    fut_util_mat = autarky_state + torch.reshape(control[future_util_mask],(n_types,n_types))
    mat_tmp = torch.unsqueeze(control[P[f"u_{1}"]:P[f"u_{1}"]+n_types] + beta*(autarky_state[0, :] + control[future_util_diag_mask]),-1) - \
        (utility_ind(torch.abs(torch.unsqueeze(shock_vec,-1) + torch.unsqueeze(control[P[f"c_{1}"]:P[f"c_{1}"]+n_types],0) - torch.unsqueeze(shock_vec,0)), reg_c, sigma) + beta*(fut_util_mat))
...
class SpecifiedModel(DPGPScipyModel):
    def __init__(self, V_guess=V_INFINITY, cfg={}, **kwargs):
        ...
        for indx_true in range(1,n_types): # true state in
            for indx_false in range(indx_true): # false state in
                self.submatrix_mask[indx_true,indx_false] = True
    ...
    def create_optimizer(self, d, p):
        ...
        return torch.optim.LBFGS(self.M[d][p].parameters(),lr = lr,line_search_fn='strong_wolfe')
    ...
    def sample(self,no_samples_=None):
        ...
        if self.epoch == 0:
            ...
            manual_pts_1 = scale_state_func((self.cfg["model"]["params"]["autarky_state"]),self.cfg)            
            manual_pts_2 = scale_state_func(torch.tensor([
                    [ 3.6789,  4.5519,  5.2942,  5.8633,  6.3302,  6.7339,  7.0942,  4.8645,  6.8732,  7.1544],
                    [ 3.8851,  4.6867,  5.4242,  5.9845,  6.4416,  6.8403,  7.1974,  7.4481,  7.7942,  7.9973],
                    [ 4.8640,  5.6563,  6.3659,  6.9105,  7.3775,  7.7797,  8.1738,  8.2920,  9.4345, 10.7660],
                    [ 5.8363,  6.5233,  7.4100,  7.8978,  8.3059,  8.7183,  9.8484,  9.6972,  11.5873, 12.2122],
                    [ 6.5963,  7.2793,  7.7470,  8.1922,  8.7734,  9.9224, 10.2450, 10.4358,  12.5565, 13.5883],
                    [ 7.7736,  8.3628,  8.9826,  9.5954, 10.3798, 10.9117, 11.7048, 11.8529,  14.0694, 14.4189],
                    [ 8.8421,  9.8529, 10.6903, 11.3839, 14.0409, 14.9638, 15.2520, 15.3130,  15.5146, 15.6564],
                    [ 9.8971, 11.2312, 13.6468, 14.4667, 15.7680, 16.2328, 16.4580, 15.9770,  16.1269, 16.2288],
                    [10.4840, 12.3074, 14.4024, 14.5922, 15.7816, 16.2457, 16.4708, 16.0149,  16.1648, 16.2666],
                    [11.0892, 12.5754, 14.7194, 15.0531, 15.8516, 16.3107, 16.5321, 16.3493,  16.4975, 16.5990],
                    [12.4149, 14.2193, 14.8694, 16.0287, 16.1998, 16.3678, 16.5345, 16.6194,  16.7557, 16.8543],
                    [14.5572, 15.1522, 15.8566, 16.3974, 16.8714, 17.1012, 17.2672, 16.6956,  16.8318, 16.9304],
                    [15.5653, 15.6914, 16.7708, 17.2927, 17.5052, 17.6610, 17.8137, 17.2860,  17.4186, 17.5163],
                    [16.9212, 17.0367, 17.1885, 17.3471, 17.5364, 17.7081, 17.8595, 17.4137,  17.5462, 17.6440],
                    [17.8707, 17.9799, 18.1235, 18.2738, 18.4239, 18.5721, 18.7176, 18.0023,  18.1339, 18.2315],
                    [18.1763, 18.2835, 18.4247, 18.5725, 18.7202, 18.8661, 19.0094, 18.8589,  18.9901, 19.0876]
                ]),self.cfg)            
            convex_hull_pts = scale_state_func(torch.tensor([[3.6789,4.55191,5.29423,5.86329,6.33018,6.7339,7.09424,7.4197,7.70555,7.90964],
            ...
            epsilon = 0.02/(1-beta)
            normal_sample = torch.normal(0., epsilon, size=(no_samples_, 10))
            ...
            state_sample_all = normal_sample + torch.from_numpy(sample_on_convex_hull(convex_hull_pts.detach().numpy(), no_samples_))
            ...
            state_sample_ = torch.cat((manual_pts_1,manual_pts_2, state_sample_all),0)
            ...
            for d in range(self.discrete_state_dim):
                ...
                mask_feas = train_v + gp_offset > lower_V
                feas_train_sample = train_sample[mask_feas,:]
                UB_state = torch.max(feas_train_sample,dim=0,keepdim=True)[0]
                LB_state = torch.min(feas_train_sample,dim=0,keepdim=True)[0]

                state_sample_01 = torch.from_numpy(sampler.random(n=int(no_samples_)))

                state_sample_ = (UB_state - LB_state)*state_sample_01 + (LB_state)
    ...
    def sample_start_pts(self, scaled_state, params, policy, n_restarts):
        ...
        autarky_state = self.cfg["model"]["params"]["autarky_state"]
            ...
            if indxp == 0:  #first batch assume we stay at current util
                for indxt1 in range(n_types):
                    for indxt2 in range(n_types):
                        ...
                        control[P[f"fut_util_{indxt1+1}_{indxt2+1}"]] = state[S[f"w_{indxt2+1}"]] - autarky_state[0,indxt2]
            else:
                for indxt1 in range(n_types):
                    for indxt2 in range(n_types):
                        norm_sample = torch.normal(0.0,0.1,(1,))
                        # control[P[f"fut_util_{indxt1+1}_{indxt2+1}"]] = 0. + norm_sample
                        control[P[f"fut_util_{indxt1+1}_{indxt2+1}"]] = state[S[f"w_{indxt2+1}"]] + norm_sample.item() - autarky_state[0,indxt2]

            control[P[f"pen_{1}"]:P[f"pen_{1}"]+n_types] = 0.
            control[P[f"pen_u_{1}"]:P[f"pen_u_{1}"]+n_types] = 0.

            control[P[f"u_{1}"]:P[f"u_{1}"]+n_types] = torch.relu(torch.sum(inv_trans_mat * torch.unsqueeze(state[S[f"w_{1}"] : S[f"w_{1}"] + n_types],0),-1) -  beta*(autarky_state[0,:] + control[self.future_util_diag_mask]))
            # control[P[f"u_{1}"]:P[f"u_{1}"]+n_types] = torch.relu(torch.sum(inv_trans_mat * torch.unsqueeze(state[S[f"w_{1}"] : S[f"w_{1}"] + n_types],0),-1) -  beta*(control[self.future_util_diag_mask]))
    ...
    def get_params(self,scaled_state,policy):
        ...
        # obj_val = (self.M[int(scaled_state[-1].item())][0].predict_mean(p_i)[0] + gp_offset)
    ...
    def post_process_optimization(self, scaled_state, params, control, value):
        ...
        pen_deviation = self.cfg["model"]["params"]["pen_deviation"]

        future_util_mask = self.future_util_mask
        S = self.S
        autarky_state = self.cfg["model"]["params"]["autarky_state"]
        # fut_util_deviation = torch.linalg.vector_norm(torch.reshape(control[future_util_mask],(n_types,n_types)), ord=2, dim=-1) ** 2 / n_types
        fut_util_deviation = torch.linalg.vector_norm(
            autarky_state + torch.reshape(control[future_util_mask],(n_types,n_types)) - torch.unsqueeze(state[S[f"w_{1}"] : S[f"w_{1}"] + n_types],0)
        , ord=2, dim=-1) ** 2 / n_types
        ...
        total_pen_dev = torch.sum(fut_util_deviation) * pen_deviation
        ...
        gp_offset = self.cfg["model"]["params"]["GP_offset"]
        out_val = value + total_pen * pen_opt_vf + total_pen_dev
    ...
    def u(self, scaled_state, params, control):
        ...
        total = torch.sum((1 - beta) * (
            ...
            - control[self.P[f"pen_{1}"]:self.P[f"pen_{1}"]+n_types]*pen_opt_vf - control[self.P[f"pen_u_{1}"]:self.P[f"pen_u_{1}"]+n_types]*pen_opt_vf) - torch.sum(fut_util_deviation) * pen_deviation
    ...
    def state_next(self, scaled_state, params, control, zpy, opt=False):
        ...
        s[S[f"w_{1}"]:S[f"w_{1}"]+n_types] = autarky_state[0, :] + control[self.P[f"fut_util_{int(zpy)+1}_{1}"]:self.P[f"fut_util_{int(zpy)+1}_{1}"] + n_types]
    ...
    def state_next_batched(self, scaled_state, params, control):
        ...
        tmp_mat = autarky_state + torch.reshape(control[self.P[f"fut_util_{1}_{1}"]:self.P[f"fut_util_{1}_{1}"] + n_types**2],[n_types,n_types])
    ...
    def lb(self, scaled_state, params):
        ...
        for indxa in range(n_types):
            for indxa2 in range(n_types):
                ...
                X_L[self.P[f"fut_util_{indxa+1}_{indxa2+1}"]] = (lower_w[indxa2]) - autarky_state[0,indxa2]
        return X_L
    ...
    def ub(self, scaled_state, params):
        ...
        for indxa in range(n_types):
            for indxa2 in range(n_types):
                ...
                X_U[self.P[f"fut_util_{indxa+1}_{indxa2+1}"]] = (upper_w[indxa2]) - autarky_state[0,indxa2]
...
##############################################################################################################################
    ...
    def BAL(self):
        ...
        if (self.epoch // self.cfg["BAL"]["epoch_freq"]) % 3 == 0: #every 20th BAL step we randomly sample instead of simulating
            if not self.cfg.get("distributed") or dist.get_rank() == 0:
                new_sample = torch.empty((0,self.state_sample_all.shape[1]+1))
                mask_feas = self.combined_sample_all[:,0] + gp_offset > lower_V + 0.3
                ...
                for indxp in range(state_sample_feas.shape[0]):
                    ...
                    for indx_eval in range(next_points.shape[0]):
                        ...
                        with torch.no_grad():
                            bal_utility[indx_eval] = (weighs[indx_eval] > 0.) * self.bal_utility_func(eval_pt,0)
            ...
            for indx_ in worker_slice: #start a simulation for each types's maximum starting point
                ...
                for indxt in range(n_sim_steps):
                    ...
                    if (v  < self.M[current_disc_state][0].min_feas_train_y  and indxt > 0):                    
                    ...
                    next_state = torch.unsqueeze(self.state_next(current_state[0,:], params, pol_out, next_disc_state),0)
                    next_state[:,:-1] = torch.maximum( #project simulation back into bounds
                        LB_state,
                        torch.minimum(
                            UB_state,
                            next_state[:,:-1]
                            ))
                    current_step_length = torch.linalg.vector_norm(next_state[:,:-1] - current_state[:,:-1], ord=2, dim=-1)
                    clipped_step_legnth = torch.minimum(0.01*torch.ones_like(current_step_length), current_step_length)
                    adjusted_next_state = next_state.clone()
                    adjusted_next_state[:,:-1] = current_state[:,:-1] + (clipped_step_legnth / current_step_length) * (next_state[:,:-1] - current_state[:,:-1])
                    current_state = adjusted_next_state.clone()
    ...
    def create_model(self, d, p, train_x, train_y, warm_start=False):
        ...
        if warm_start:
            ...
            if p == 0:
                ...
                upper_w = self.cfg["model"]["params"]["upper_w"]
                scale_bound = scale_state_func(upper_w,self.cfg)
                length_scale_init = scale_bound * torch.sqrt(torch.tensor(1.*n_types))
    ...
    def solve_all(self, n_restarts=1):
        policies = []
        self.V_sample = torch.zeros(self.state_sample.shape[0])
        for s in range(self.state_sample.shape[0]):
            try:
                if self.feasible[s] == 1.0:
                    v, p = self.solve(self.state_sample[s, :],self.combined_sample[s,1:], n_restarts)
                ...
                with torch.no_grad():
                    ...
                    self.V_sample[s] = (
                        (self.M[d][0].predict_mean(torch.unsqueeze(sample, 0)) )
                    )
