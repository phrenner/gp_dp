...
from DPGPScipyModel import DPGPScipyModel, ScipyModel, ScipyModel_max_vf
...
def V_INFINITY(model, scaled_state):
    ...
    disc_state = state[-2].type(torch.IntTensor)
    ...
    if model.epoch < n_feas_set_it:
        ...
        gp_offset = model.cfg["model"]["params"]["GP_offset"][disc_state]
        mult_fac = 1.0
...
class SpecifiedModel(DPGPScipyModel):
    ...
    def get_solver_config(self,d,p):
        if p > 0:
            ...
            lr = 0.01 #self.cfg["torch_optim"].get("lr_vf",1e-3)
    ...
    def what_pol_to_fit(self): #per default only fit the VF in each iteration; customize if we need some policies
        ...
        for indxa1 in range(n_types):
            for indxa2 in range(n_types):
                fit_lst.append((0,1 + self.P[f"fut_util_{indxa1+1}_{indxa2+1}"]))
                
        for indxa in range(n_types):
            fit_lst.append((0,1 + self.P[f"c_{indxa+1}"]))
    ...
    def sample(self,no_samples_=None):
        ...
        state_sample = torch.zeros([0,n_types+2])
            ...
            state_sample = torch.cat(
                (
                    ...
                    torch.cat(
                        (
                            state_sample_,
                            torch.ones(no_samples,1) * indx_type,
                            torch.zeros(no_samples,1)
                    ),dim=1)
                ),
                dim=0,
            )
    ...
    def sample_start_pts(self, scaled_state, params, policy, n_restarts):
        ...
        for indxp in range(n_pts):
            ...
            if indxp == 0:  #first batch assume we stay at current util
                # control[P[f"fut_util_{1}_{1}"]:P[f"fut_util_{1}_{1}"]+n_types**2] = 0.
                control[P[f"fut_util_{1}_{1}"]:P[f"fut_util_{1}_{1}"]+n_types**2] = (torch.unsqueeze(state[S[f"w_{1}"]:S[f"w_{1}"]+n_types],0) * torch.ones((n_types,n_types))).flatten()
    ...
    def is_feasible(self, scaled_state,value,control):
        ...
        disc_state = scaled_state[-2].type(torch.IntTensor)
        gp_offset = self.cfg["model"]["params"]["GP_offset"][disc_state]
    ...
    def u(self, scaled_state, params, control):
        state = self.unscale_state(scaled_state)
        n_types = self.cfg["model"]["params"]["n_types"]
        disc_state_in = (state[-2]).type(torch.IntTensor)
    ...
    def E_V(self, scaled_state, params, control):
        ...
        obj_vec = weights * (self.M[0][0](points[:,:-1]).mean + gp_offset)
        e_v_next = torch.sum(obj_vec)
        # e_v_next -= torch.sum(1000 * pen_opt_vf * (torch.nn.functional.relu(lower_V - obj_vec))**2)
    ...
    def state_next(self, scaled_state, params, control, zpy, opt=False):
        ...
        # s[S[f"w_{1}"]:S[f"w_{1}"]+n_types] = state[self.S[f"w_{int(zpy)+1}"]] + control[self.P[f"fut_util_{int(zpy)+1}_{1}"]:self.P[f"fut_util_{int(zpy)+1}_{1}"] + n_types]
    ...
    def state_next_batched(self, scaled_state, params, control):
        ...
        # update discrete state
        s[:,-2] = 1.* torch.arange(n_types)
        
        tmp_mat = torch.reshape(control[self.P[f"fut_util_{1}_{1}"]:self.P[f"fut_util_{1}_{1}"] + n_types**2],[n_types,n_types])
        # s[:,S[f"w_{1}"]:S[f"w_{1}"]+n_types] = torch.unsqueeze(state[self.S[f"w_{1}"]:self.S[f"w_{1}"]+n_types],-1) + tmp_mat
    ...
    def lb(self, scaled_state, params):
        ...
        # for indxa in range(n_types):
        #     for indxa2 in range(n_types):
        #         # X_L[self.P[f"fut_util_{indxa+1}_{indxa2+1}"]] = (lower_w[indxa2] - state[self.S[f"w_{indxa + 1}"]])
    ...
    def ub(self, scaled_state, params):
        ...
        # for indxa in range(n_types):
        #     X_U[self.P[f"u_{indxa+1}"]] = utility_ind(100 * upper_transfer, reg_c, sigma)
        #     X_U[self.P[f"pen_{indxa+1}"]] = 1000.
        #     X_U[self.P[f"pen_u_{indxa+1}"]] = 1000.
        #     X_U[self.P[f"c_{indxa+1}"]] = 100 * upper_transfer
        #     for indxa2 in range(n_types):
        #         # X_U[self.P[f"fut_util_{indxa+1}_{indxa2+1}"]] = (upper_w[indxa2] - state[self.S[f"w_{indxa + 1}"]])
...
##############################################################################################################################
    ...
    def BAL(self):
        ...
        lower_w = self.cfg["model"]["params"]["lower_w"]
        upper_w = self.cfg["model"]["params"]["upper_w"]
        LB_state = torch.zeros(dim_state - 2)
        UB_state = torch.zeros(dim_state - 2)
        ...
        for indxd in range(n_types):
            ...
            mask = self.state_sample_all[:, -2] == indxd * torch.tensor(1.)
            V_sample = self.V_sample_all[mask]
            max_ind_opt = torch.argmax(V_sample)

            sample_tmp = self.state_sample_all[mask,:]
            try:
                obj,point = self.find_max_of_vf(sample_tmp[max_ind_opt,:-1],LB_state,UB_state)    #run optimization to find maximum of current VF
        ...
        for indx_ in worker_slice: #start a simulation for each types's maximum starting point
            ...
            for indxt in range(n_sim_steps):
                ...
                try:
                    for p1 in range(1,n_types+1):
                        ...
                        for p2 in range(1,n_types+1):
                            with torch.no_grad():
                                next_pol_tmp = self.M[0][1+P[f"fut_util_{p1}_{p2}"]](current_state[:,:-1]).mean
                ...
                with torch.no_grad():
                    ...
                    v = self.M[0][0](current_state[:,:-1]).mean

                #check of we need to abort sim because we walked somewhere nonsensical
                if (v + gp_offset[current_disc_state] <= lower_V and indxt > 0 and self.epoch > n_rand_its):
                    print(f"Break simulation in iteration {indxt} with value {v + gp_offset[current_disc_state]} at point {current_state[0,:]}")
                    break
                ...
                current_state[:,:-2] = torch.maximum( #project simulation back into bounds
                    LB_state,
                    torch.minimum(
                        UB_state,
                        current_state[:,:-2]
    ...
    def eval_f_max_vf(self, disc_state, control):
        out = torch.zeros(1)
        input = torch.cat((control,torch.tensor([disc_state],dtype=torch.float32)),dim=0)
        out -= self.M[0][0](torch.unsqueeze(input,dim=0)).mean + self.M[0][0](torch.unsqueeze(input,dim=0)).variance
        return out
    ...
    def find_max_of_vf(self, start_pt,LB_var,UB_var):
        X_L = LB_var
        X_U = UB_var

        disc_state = start_pt[-1].item()
        ...
        x_init = start_pt[:-1].clone().detach().numpy()
