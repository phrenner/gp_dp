...
def process(m, cfg):
    ...
    for indxd in range(n_types):
        ...
        # elif m.epoch < m.cfg["n_feas_set_it"]:
        #     mask_pos = m.V_sample[mask] + gp_offset > 0.0
        #     n_pos_pts = torch.sum(mask_pos)
        #     print(f"Number of positive points in state {indxd} is {n_pos_pts}")
            ...
            for indxp in range(1, m.policy_dim + 1):
                pol_out[indxp - 1] = torch.minimum(
                    ...
                    torch.maximum(
                        ...
                        (m.M[current_disc_state][indxp](current_state[:, :-1]).mean)))[0]
