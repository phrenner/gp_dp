...
def V_INFINITY(model, scaled_state):
    cfg = model.cfg
    state = model.unscale_state(scaled_state)[...,:cfg["model"]["params"]["n_types"]]
    return V_INFINITY_(state, cfg, disc_state=int(scaled_state[...,-1])).item()


def V_INFINITY_(state, cfg, disc_state=None):
    n_types = cfg["model"]["params"]["n_types"]
    trans_mat = cfg["model"]["params"]["trans_mat"]
    shock_vals = cfg["model"]["params"]["shock_vec"]
    ...
    for indxa in range(n_types):
        sum_tmp += trans_mat[disc_state,indxa]*(
            shock_vals[indxa] - inv_utility_ind((1 - beta) * state[...,indxa],reg_c,sigma))
...
class SpecifiedModel(DPGPScipyModel):
    ...
    def sample(self,no_samples_=None):
        ...
        if self.epoch == 0:
            manual_pts = torch.tensor([
                [18.07856900381287, 18.844093924313952], 
                [7.071779312357953, 12.729210762388316], 
                [7.401975882761775, 9.09305218922801], 
                [4.137998707602195, 6.683973820051117], 
                [8.663146827328422, 10.150173477863829], 
                [14.589212923164576, 15.525503937954547], 
                [8.380559650889554, 9.909274578024995], 
                [10.771257589667144, 12.004094463345385], 
                [12.68010311294318, 13.745195759525112], 
                [14.32520231128996, 15.27749756114766], 
                [15.795319310470912, 16.664805614514275], 
                [17.137738558600432, 17.94300435147963], 
                [19.54561396845826, 20.25631197363793]
            ])

            state_sample = torch.cat([
                scale_state_func(self.cfg["model"]["params"]["autarky_state"],self.cfg),
                # scale_state_func(manual_pts,self.cfg),
                state_sample
            ],0)
        else:
    ...
    def sample_start_pts(self, scaled_state, params, policy, n_restarts):
        ...
        for indxp in range(n_pts):
            ...
            if indxp == 0:  #first batch assume we stay at current util
                ...
                for indxt1 in range(n_types):
                    for indxt2 in range(n_types):
                        ...
                        control[P[f"fut_util_{indxt1+1}_{indxt2+1}"]] = state[S[f"w_{indxt2+1}"]] + norm_sample.item()
            ...
            # control[P[f"u_{1}"]:P[f"u_{1}"]+n_types] = torch.relu(torch.sum(inv_trans_mat * torch.unsqueeze(state[S[f"w_{1}"] : S[f"w_{1}"] + n_types],0),-1) -  beta*(state[S[f"w_{1}"] : S[f"w_{1}"] + n_types] + control[self.future_util_diag_mask]))
    ...
    def get_params(self,state,policy):
        ...
        # p_i = torch.unsqueeze(state[:-1], 0)
        # obj_val = (self.M[int(state[-1].item())][0].predict_mean(p_i)[0] + gp_offset)
...
##############################################################################################################################
    ...
    def BAL(self):
        ...
        if (self.epoch // self.cfg["BAL"]["epoch_freq"]) % 10000 == 0: #every 20th BAL step we randomly sample instead of simulating
            if not self.cfg.get("distributed") or dist.get_rank() == 0:
                ...
                for indxt in range(n_types):
                    ...
                    with torch.no_grad():
                        V_val_vec = self.M[indxt][0].predict_mean(rand_sample[mask_type,:-1])
                    mask_feas = V_val_vec > lower_V - 0.1 - gp_offset
                    sample_tmp = rand_sample[mask_type,:][mask_feas,:]
                    n_pts = sample_tmp.shape[0]
                    bal_util = -1e10
                    pts_tmp = torch.zeros([1,1 + dim_state])
                    for indxp in range(n_pts):
                        with torch.no_grad():
                            bal_util_tmp = self.bal_utility_func(sample_tmp[indxp,:],0,pen_val=torch.tensor([pen_val]))
        
                        if bal_util_tmp > bal_util:
                            pts_tmp[:,:-1] = sample_tmp[indxp,:]
                            pts_tmp[:,-1] = bal_util_tmp
                            bal_util = bal_util_tmp
            ...
            self.fit_GP(self.only_fit_trans_pol(), True, compute_metrics=False) 
            n_sim_steps = 1000
            P = self.P
            trans_mat = self.cfg["model"]["params"]["trans_mat"]
            beta = self.cfg["model"]["params"]["beta"]
            ...
            for indx_ in worker_slice: #start a simulation for each types's maximum starting point
                ...
                for indxt in range(n_sim_steps):
                    if indx_ < n_types and indxt < n_sim_steps:
    ...
    def iterate(self):
        ...
        indx_it = 0
        error_old = 1
        while indx_it < self.cfg["model"]["params"]["n_Howard_steps"] and error_old > 1e-5 and self.epoch > 100:
    ...
    def create_model(self, d, p, train_x, train_y, warm_start=False):
        if self.cfg.get('use_fixed_noise',True):
            if p == 0:
                gp_offset = self.cfg["model"]["params"]["GP_offset"]
                lower_V = self.cfg["model"]["params"]["lower_V"]
                mask_infes = train_y + gp_offset < lower_V 
                noise_vec = torch.ones(train_y.shape[0]) * 1e-5
                noise_vec[mask_infes] = 1e-5
            else:
                noise_vec = torch.ones(train_y.shape[0]) * 1e-5
