...
def grad_V(m,d,state):
    ...
    outputs = V_fun.predict_mean(inputs)
...
def process(m, cfg, checkpoint_indx_start, checkpoint_indx):
    ...
    for indxd in range(n_types):
        ...
        # pts_out = torch.cat(
        #     (
        #         (m.state_sample_all[mask, :][:, :-1]),
        #         m.combined_sample_all[mask, :]
        #     ), dim=1)
        # np.savetxt(
        #     file_out,
        #     pts_out.numpy(),
...
def compare(m1, m2, cfg, checkpoint_indx_start, checkpoint_indx):
    ...
    with open(f"V_func_error_{checkpoint_indx}.txt", 'a') as csvfile:
        np.savetxt(
            csvfile,
            error_out,
            delimiter=' ',
...
def simulate(m, m_prev, cfg, model, shock_vec=None):
    ...
    for indxt in range(n_sim_steps):
        ...
        out_np[0, :dim_state - 1] =  ((model.unscale_state_func(torch.from_numpy(out_np[0,:dim_state - 1]),m.cfg)).numpy())  * (1 - sigma_util)
        np.savetxt(
            sim_out,
            out_np,
            fmt='%.18e',
