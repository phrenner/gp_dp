...
def process(m, cfg, checkpoint_indx_start, checkpoint_indx):
    ...
    for indxd in range(n_types):
        mask_state = plot_sample[:,-1] == 1.*indxd
        plot_sample_d = plot_sample[mask_state,:-1]
        V_d = m.M[indxd][0].eval()
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            V_p = torch.maximum(torch.tensor(0.),V_d.predict_mean(plot_sample_d))
        
        np.savetxt(
            open(f"V_sample_{checkpoint_indx_start}_{checkpoint_indx}_DS_{indxd}.txt", 'w'),
            torch.cat([plot_sample_d,torch.unsqueeze(V_p,1)],1).numpy(), 
            fmt='%.18e',
            delimiter=' ',
            newline='\n',
            header='',
    ...
    for indxd in range(len(out_lst)):
        scale_x = m.M[out_lst[indxd][0]][out_lst[indxd][1]].scale_x
        offset_x = m.M[out_lst[indxd][0]][out_lst[indxd][1]].offset_x
        scale_y = m.M[out_lst[indxd][0]][out_lst[indxd][1]].scale_y
        offset_y = m.M[out_lst[indxd][0]][out_lst[indxd][1]].offset_y
        print(f"State {out_lst[indxd]} scale_x: {scale_x} offset_x: {offset_x} scale_y: {scale_y} offset_y: {offset_y}")
...
def compare(m1, m2, checkpoint_indx_start, checkpoint_indx, init_run=False, sim_path=None):
    """Compare two GP-s"""
    ...
    for indxd in range(m1.cfg["model"]["params"]["discrete_state_dim"]):
        ...
        if sim_path is not None:
            sim_path_d = sim_path[sim_path[:,-1] == 1.*indxd,:-1]
        V1 = m1.M[indxd][0].eval()
        V2 = m2.M[indxd][0].eval()
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            V_p1 = V1.predict_mean(state_sample)
            V_p2 = V2.predict_mean(state_sample)
            if sim_path is not None:
                sim_V_p1 = V1.predict_mean(sim_path_d)
                sim_V_p2 = V2.predict_mean(sim_path_d)
            else:
                sim_V_p1 = torch.tensor([0.0])
                sim_V_p2 = torch.tensor([0.0])
        ...
        error_vec += np.array([[(1 / V_p1.shape[0]**0.5 * torch.linalg.norm((V_p1 - V_p2)/(1 + torch.abs(V_p2)))).numpy(),
                                ...
                                (1 / sim_V_p1.shape[0]**0.5 * torch.linalg.norm((sim_V_p1 - sim_V_p2)/(1 + torch.abs(sim_V_p2)))).numpy(),
                                (torch.linalg.norm((sim_V_p1 - sim_V_p2)/(1 + torch.abs(sim_V_p2)),ord=float('inf'))).numpy(),
                            ]])

    error_out = np.zeros((1, 6))
    error_out[0, 2:] = error_vec[0, :]/m1.cfg["model"]["params"]["discrete_state_dim"]
    error_out[0,0] = m1.epoch
    error_out[0,1] = m2.epoch

    if init_run:
        header = '#T1 T2 L2 LInf sim_L2 sim_LInf'
        mode = 'w'
    else:
        header = ''
        mode = 'a'
    if checkpoint_indx_start > 0:
        file_name = f"V_func_error_{checkpoint_indx}.txt"
    else:
        file_name = f"V_func_error_{checkpoint_indx_start}_{checkpoint_indx}.txt"
...
def simulate(m, m_prev, cfg, model, shock_vec=None):

    torch.manual_seed(1054211) #set seed for reproducible results
    np.random.seed(1054211)
    n_sim_steps = 10000
    ...
    sim_path = torch.zeros([n_sim_steps, dim_state])
    l1_err = 0
    l2_err = 0
    for indxt in range(n_sim_steps):
        ...
        sim_path[indxt, :] = current_state[0,:]
        current_disc_state = int(current_state[0, -1].item())
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            pred_m = m.M[current_disc_state][0].predict_mean(
                current_state[:, :-1])
            out_np[0, dim_state] = (pred_m + gp_offset)[0].numpy()  # value function

            out_np[0, dim_state+1] = 0
            out_np[0, dim_state+2] = 0

            abs_diff = (
                np.abs((m_prev.M[current_disc_state][0].predict_mean(
                current_state[:, :-1]) + gp_offset).numpy() - out_np[0, dim_state])/
                (1 + np.abs(out_np[0, dim_state])))  # relative abs of difference of vf in this and the prior epoch

            l1_err += abs_diff
            l2_err += abs_diff**2

            out_np[0, dim_state+3] = abs_diff[0]
            ...
            for indxp in range(1, m.policy_dim + 1):
                pol_out[indxp - 1] = torch.minimum(
                    ...
                    torch.maximum(
                        ...
                        (m.M[current_disc_state][indxp].predict_mean(current_state[:, :-1]))))[0]
        ...
        out_np[0, :dim_state - 1] =  ((torch.from_numpy(out_np[0,:dim_state - 1]))).numpy()
        np.savetxt(
            sim_out,
            out_np,
            fmt='%.18e',
        ...
        if shock_vec is not None and indxt < shock_vec.shape[0] - 1:
            next_disc_state = shock_vec[indxt+1]
        else:
