...
class DPGPModel(ABC):
    ...
    def sync_GP(self, d: int = 0, p: int = 0, source_rank: int = 0):
        if self.cfg.get("distributed"):
            b_objects = self.M[d][p].state_dict()
            keys = list(b_objects.keys())
            values = list(b_objects.values())
            n_values = len(values)
            if dist.is_initialized():
                dist.barrier()
            
            for indx in range(n_values):
                values[indx] = dist.broadcast(values[indx], src=source_rank)

            if dist.get_rank() != source_rank:
                self.M[d][p].load_state_dict({keys[indx]: values[indx] for indx in range(n_values)})
    ...
    def fit_GP(self, dp: List[Tuple] = [()], warm_start=False):
    ...
    def optimize_ll(self,training_iter,d,p,train_sample,train_v):
        ...
        for i in range(1, 1 + training_iter):
            ...
            rel_ll_grad = torch.abs(ll_grad_norm)/max(1, torch.abs(current_ll))
            ...
            rel_ll_change = abs(current_ll - old_ll)/(max(abs(current_ll),abs(old_ll),1))
    ...
    def iterate(self):
        ...
        if self.epoch > 0:
            self.fit_GP(self.what_to_fit(), self.warm_start)
        else:
            self.fit_GP(self.what_to_fit())
        ...
        for s in range(self.state_sample.shape[0]):
            try:
                ...
                if s % (self.state_sample.shape[0] / 10) == 0:
                    main_logger.debug(f"Finished solving Optimization Problem #{s}")
    ...
    def load(cls, path, cfg_override, **kwargs):
        ...
        for d in range(dpgp.discrete_state_dim):
            for p in range(1 + dpgp.policy_dim):
                ...
                model_state_dict = checkpoint["model_state_dict"][f"{d},{p}"]
                try:
                    dpgp.M[d][p].load_state_dict(model_state_dict)
                except RuntimeError as exc:
                    # Allow loading checkpoints across small GPModel definition changes.
                    missing_keys, unexpected_keys = dpgp.M[d][p].load_state_dict(
                        model_state_dict, strict=False
                    )
                    main_logger.warning(
                        f"Non-strict model load for d={d}, p={p} after strict load failed: {exc}. "
    ...
    def scatter_sample(self):
        if self.cfg.get("distributed"):
            ...
            if dist.get_rank() != 0:
                n_rows = int(self.state_sample_all_shape[0].item())
                n_cols = int(self.state_sample_all_shape[1].item())

                # Keep dtype/device aligned with rank-0 tensors to avoid MPI truncation.
                self.state_sample_all = self.state_sample_all.new_zeros((n_rows, n_cols))
                self.combined_sample_all = self.combined_sample_all.new_zeros((n_rows, 1 + self.policy_dim))
                self.feasible_all = self.feasible_all.new_zeros((n_rows,))
                self.non_converged_all = self.non_converged_all.new_zeros((n_rows,))
