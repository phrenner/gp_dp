...
class DPGPScipyModel(DPGPModel):
    ...
    def solve(self, state, policy, n_restarts=2):
    ...
    def eval_f_max_vf(self, disc_state, control):
        out = torch.zeros(1)
        out -= self.M[disc_state][0](torch.unsqueeze(control,dim=0)).mean + self.M[disc_state][0](torch.unsqueeze(control,dim=0)).variance
        return out
