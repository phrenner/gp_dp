        ...
        for indx_false in range(indx_true,n_types): # false state in
            G[counter] = 100 * torch.max(
                ...
                (utility_ind(
                    ...
                    reg_c, 
                    sigma) + beta*(control[P[f"fut_util_{indx_false+1}_{indx_true+1}"]])))) + 0.000001
            counter += 1
...
class SpecifiedModel(DPGPScipyModel):
    ...
    def sample(self,no_samples_=None):
        ...
        if self.epoch == 0:
            manual_pts = torch.tensor([[108.1757781653268, 113.04373329581152], [263.30935511288004, \
                264.90569326898515], [182.93995956187158, 191.17230285519042], \
                [262.9505930117554, 264.55104671709216], [157.37159636766904, \
                160.21966797116403], [185.31732192333928, 187.6655946598933], \
                [208.73677572787798, 210.79045589593304], [229.48841080073265, \
                231.33910313955755], [248.3768743290814, 250.07599642431097], \
                [265.85337939147246, 267.4334727215618], [282.2059141508095, \
                283.6892045192478], [297.63340441423253, 299.0359042378734], \
                [312.2808493185358, 313.6145622945319], [326.2581697229043, \
                327.5323764767448]])
...
##############################################################################################################################
    ...
    def BAL(self):
        ...
        if (self.epoch // self.cfg["BAL"]["epoch_freq"]) % 20 == 0: 
    ...
    def iterate(self):
        ...
        if self.epoch == 0:
            ...
            fit_lst = self.only_fit_VF()
            self.cfg["scipyopt"]["no_restarts"] = 2
