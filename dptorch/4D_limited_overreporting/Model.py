...
def EV_G_ITER(model, scaled_state, params, control):
    ...
    M = 2*n_types + 2* n_types*(n_types-1) // 2   # number of constraints
    disc_state = state[-1].type(torch.IntTensor)
    ...
    for indx in range(n_types):
        G[counter] = (1 - beta) * (state[S[f"w_{indx+1}"]] + control[P[f"pen_{indx+1}"]] - control[P[f"pen_u_{indx+1}"]] - \
                sum(
                    [
    ...
    for indx_true in range(1,n_types): # true state in
        for indx_false in range(indx_true,n_types): # false state in
            G[counter] = 1 * control[P[f"mu_{indx_false+1}_{indx_true+1}"]] * (
                    (control[P[f"u_{indx_true+1}"]] + beta*(control[P[f"fut_util_{indx_true+1}_{indx_true+1}"]]) - 
                (utility_ind(
                    torch.max(torch.tensor(0.),shock_vec[indx_true] + control[P[f"c_{indx_false+1}"]] - shock_vec[indx_false]),
                    reg_c, 
                    sigma) + beta*(control[P[f"fut_util_{indx_false+1}_{indx_true+1}"]])))) + 0.0000001
            counter += 1
...
class SpecifiedModel(DPGPScipyModel):
    def __init__(self, V_guess=V_INFINITY, cfg={}, **kwargs):
        ...
        for indx_true in range(1,n_types): # true state in
            for indx_false in range(indx_true,n_types): # false state in
                policy_names += [f"mu_{indx_false+1}_{indx_true+1}"]
        ...
        n_types = cfg["model"]["params"]["n_types"]
        control_dim_loc = 4 * n_types + n_types**2  + n_types*(n_types-1) // 2 
        super().__init__(
    ...
    def sample(self,no_samples_=None):
        ...
        if self.epoch == 0 and n_types == 4:
            ...
            manual_pts = scale_state_func(torch.tensor([
                ...
                [137.249, 137.707, 139.53, 139.843], 
            ...
            convex_hull_pts = scale_state_func(torch.tensor([
                ...
                [220.258, 222.105, 225.33, 225.801], 
                [116.92, 117.638, 119.974, 120.352], 
                [116.15, 116.891, 119.261, 119.644], 
                [137.249, 137.707, 139.53, 139.843], 
                [120.124, 120.769, 122.986, 123.349], 
                [115.088, 115.866, 118.291, 118.681],
                [156.07, 156.435, 157.99, 158.267], 
                [183.373, 183.662, 184.955, 185.194], 
    ...
    def sample_start_pts(self, scaled_state, params, policy, n_restarts):
        ...
        for indxp in range(n_pts):
            ...
            if indxp == 0:  
                ...
                for indxt1 in range(n_types):
                    torch.manual_seed(indxp*123 + self.epoch*123)
                    normal_noise = torch.randn(n_types) * self.M[indxt1][0].std_x * 0.1
                    for indxt2 in range(n_types):
                        control[P[f"fut_util_{indxt1+1}_{indxt2+1}"]] = state[S[f"w_{indxt2+1}"]] + normal_noise[indxt2]
            ...
            inv_mat = self.cfg["model"]["params"]["trans_mat_inv"]
            u = torch.squeeze(inv_mat @ torch.unsqueeze(state[:n_types],1)) - beta * control[self.future_util_diag_mask]
            for indxr in range(n_types):
                control[P[f"u_{indxr+1}"]] = u[indxr]
                control[self.P[f"c_{indxr+1}"]] = inv_utility_ind(control[P[f"u_{indxr+1}"]] ,reg_c,sigma)

            shock_vec=self.cfg["model"]["params"]["shock_vec"]
            for indx_true in range(1,n_types): 
                for indx_false in range(indx_true,n_types): 
                    if shock_vec[indx_true] + control[P[f"c_{indx_false+1}"]] - shock_vec[indx_false] <= 0:
                        control[P[f"mu_{indx_false+1}_{indx_true+1}"]]  = 0.
    ...
    def post_process_optimization(self, scaled_state, params, control, value):
        ...
        min_GP_value = self.cfg["model"]["params"]["min_GP_value"]
        out_val_adj = torch.clamp(out_val, min=min_GP_value, max=upper_V) - gp_offset
    ...
    def ub(self, scaled_state, params):
        ...
        for indxa in range(n_types):
            ...
            X_U[self.P[f"pen_{indxa+1}"]] = 100*upper_w[indxa] * params["use_penalty"]
            X_U[self.P[f"pen_u_{indxa+1}"]] = 100*upper_w[indxa] * params["use_penalty"]
        ...
        for indx_true in range(1,n_types): # true state in
            for indx_false in range(indx_true,n_types): # false state in
                X_U[self.P[f"mu_{indx_false+1}_{indx_true+1}"]] = 100 * upper_transfer

        return X_U

    def cl(self, scaled_state, params):
        n_types = self.cfg["model"]["params"]["n_types"]
        M = 2*n_types + 2 * n_types*(n_types-1) // 2
        G_L = np.empty(M)
        G_L[:] =  0.0
        return G_L
...
##############################################################################################################################
        ...
        if (self.epoch // self.cfg["BAL"]["epoch_freq"]) % 10000 == 0: #every 20th BAL step we randomly sample instead of simulating
            ...
            for indx_ in worker_slice: #start a simulation for each types's maximum starting point
                ...
                for indxt in range(n_sim_steps):
                    ...
                    current_state = torch.unsqueeze(self.state_next(current_state[0,:], None, pol_out, next_disc_state),0)
    ...
    def iterate(self):
        ...
        if self.epoch == 0:
            ...
            self.cfg["scipyopt"]["no_restarts"] = 6
        ...
        indx_it = 0
        error_old = 1
        while indx_it < self.cfg["model"]["params"]["n_Howard_steps"] and error_old > 1e-7 and self.epoch > 30:
    ...
    def set_train_data(self, d, p, warm_start=False):
        ...
        if p == 0:
            ...
            gp_offset = self.cfg["model"]["params"]["GP_offset"] 
            min_GP_value = self.cfg["model"]["params"]["min_GP_value"]
            mask_zero = (train_v + gp_offset <= min_GP_value)
